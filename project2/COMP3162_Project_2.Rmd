...
```{r}
data.consumer <- consumer_data
data.consumer.bev <-  subset(data.consumer, data.consumer$X=="Beverages")

data.consumer.bev <-  subset(data.consumer.bev, data.consumer.bev$Country!="k")


consumer.countries <- table(data.consumer.bev$Country)
consumer.countries <- as.data.frame(consumer.countries)

consumer.countries <- subset(consumer.countries, consumer.countries$Var1!="k")
...
for(i in 1:NROW(consumer.countries$Var1)){
  ...
  consumer.countries$avg_spend[i] <- mean(as.numeric(part_data$Total.Cost))
}
```
...
```{r}
highest_spending <- data.consumer.bev$Country[match(max(as.numeric(data.consumer.bev$Total.Cost)), as.numeric(data.consumer.bev$Total.Cost))]
...
print(paste("The country that consumes the most beverage is : ", highest_spending))
```
...
for(i in 1:NROW(consumer.countries$Var1)){
  country_name = toString(consumer.countries$Var1[i])
  part_data = subset(data.consumer.bev, data.consumer.bev$Country==country_name)
  consumer.countries$avg_profit[i] <- mean(as.numeric(part_data$Total.Profit))
}
```
...
data.consumer.bev.yearOnly$Order.Date <- as.integer(format(as.Date(data.consumer.bev$Order.Date, "%m/%d/%Y"), format = "%Y"))
...
for(i in 1:NROW(bev_yearly_rev$Var1)){
  ...
  bev_yearly_rev$revenue[i] <- sum(as.numeric(data$Total.Revenue))
...
```{r}
earliest_date <- max(as.integer(format(as.Date(data.consumer.bev$Order.Date, "%m/%d/%Y"), "%Y%m%d")),  na.rm = TRUE)
earliest_date <- as.Date(toString(earliest_date), "%Y%m%d")

analysis_date <- earliest_date %m-% months(6)

data.consumer.bev.filter <- filter(data.consumer.bev,(as.Date(data.consumer.bev$Order.Date, "%m/%d/%Y") >= analysis_date ))

data.consumer.bev.filter$rOrder.Date <- as.Date(data.consumer.bev.filter$Order.Date,"%m/%d/%Y")

data.consumer.bev.filter$Total.Revenue.num <- as.numeric(data.consumer.bev.filter$Total.Revenue)
...
for(i in 1:6){
  ...
  fil_data <- filter(data.consumer.bev,(as.Date(data.consumer.bev$Order.Date, "%m/%d/%Y") >= start_mth & as.Date(data.consumer.bev$Order.Date, "%m/%d/%Y") < end_mth ))
  avg_rev <- sum(as.integer(fil_data$Total.Revenue))
...
plot(data.consumer.bev.filter$Total.Revenue.num~data.consumer.bev.filter$rOrder.Date,type="p", col="green")
...
data.consumer.bev.m_n_d$m_n_d <- format(as.Date(data.consumer.bev$Order.Date, "%m/%d/%Y"), "%m%d")
...
winter_data <- filter(data.consumer.bev,  data.consumer.bev.m_n_d$m_n_d>= "1201" & data.consumer.bev.m_n_d$m_n_d<="0229")
...
season_cor_unit$units_sold[1] <- sum(as.numeric(spring_data$Units.Sold))
season_cor_unit$units_sold[2] <- sum(as.numeric(summer_data$Units.Sold))
season_cor_unit$units_sold[3] <- sum(as.numeric(autumn_data$Units.Sold))
season_cor_unit$units_sold[4] <- sum(as.numeric(winter_data$Units.Sold))
